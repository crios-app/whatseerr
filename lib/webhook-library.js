 ...
 * @returns {Object|null}
 */
function parseLibraryBody(rawBody, contentType) {
...
function formatItemMessage(item, is4k) {
  const fourK = is4k ? ' (4K)' : '';
  if (item.kind === 'episode') {
...
export async function handleLibraryWebhook(cfg, wahaClient, payload, logger) {
  ...
  for (const item of items) {
    ...
    for (const sub of subscribers) {
      ...
      const message = formatItemMessage(item, sub.is4k);
      if (!message) continue;
